...
def agent_8(grid, target, dim):
    ...
    while True:
        ...
        while flag==0: #Finding the cell having the maximum probability of finding the target
            ...
            flag = 1
            h = [[0 for y in range(dim)] for x in range(dim)] #Finding Manhattan distances
            for x in range(dim):
                for y in range(dim):
                    if pf[x][y] != 0:
                        h[x][y] = dist(x,y,i,j)/(pf[x][y] * dim * 2) #Dividing manhattan distances with probaility*100 to make new heuristic
                    else:
                        h[x][y] = dim ** 2
            h[i][j] = dim ** 2
            mh = min([min(x) for x in h]) #Finding value of maximum probaility
            max_con = []
            for x in range(dim):
                for y in range(dim):
                    if h[x][y] == mh:
